...
class Butterfly extends Player {
    constructor(x, y) {
        super(x, y);
        ...
        this.oldPoint = new Player(x, y);
        this.newPoint = new Player(x, y);
    }
    ...
    getCharAtScreen(point, screen) {
        if ((point.x >= 0 && point.x < screen[0].length) && (point.y >= 0 && point.y < screen.length)) {
            return screen[point.y][point.x];
    ...
    update(screen) {
        ...
        for (let i = 0; i < 4; i++) {
            points[i] = this.move(i);
            neighbors[i] = this.getCharAtScreen(points[i], screen);
        ...
        let left = this.ccw(this.dir);
        if (neighbors[left] === ' ') {
            this.oldPoint = new Player(this.x, this.y);
            this.newPoint = this.move(left);

            this.dir = left;
        }
        else if (neighbors[this.dir] === ' ') {
            this.oldPoint = new Player(this.x, this.y);
            this.newPoint = this.move(this.dir);
        }
        else {
            this.dir = this.cw(this.dir);
        }
...
class AI {
    ...
    updateButterflyList(bf) {
        var_dump(bf);
        let found = false;
        for (let i = 0; i < this.butterflyList.length; i++) {
            let dist = this.distance(this.butterflyList[i].oldPoint, bf);
            var_dump(dist);
            if (dist === 0) {
                found = true;
            }
        ...
        if (!found) {
            this.butterflyList.push(new Butterfly(bf.x, bf.y));
        }

        if (this.butterflyList.length > 3) {
            exit;
        }
    }
    ...
    initGrid() {
            ...
            for (let x = 0; x < this.width; x++) {
                switch (this.screen[y][x]) {
                    ...
                    case 'A':
                        ...
                        let underPlayer = this.player.move(DOWN);

                        if (this.isRoundFalling(this.player) && this.isDownStuck(underPlayer)) {
                    ...
                    case '-':
                        ...
                        //this.updateButterflyList(butterfly);

                        butterfly.type = 1;

                        let neighbors = this.grid.getOrtoNeighbors(butterfly, 1);
        ...
        /*
                for (let i = 0; i < this.butterflyList.length; i++) {
                    let bf = this.butterflyList[i].update(this.screen);
                    let neighbors = this.grid.getOrtoNeighbors(this.butterflyList[i], 1);
                    for (let n = 0; n < neighbors.length; n++) {
                        this.grid.setWalkableAt(neighbors[n].x, neighbors[n].y, false);
                    }
                }*/
    ...
    isRoundFalling(point) {
        let up = point.move(UP);
        if (!this.grid.isNodeInside(up)) {
            return false;
        }

        if (this.getCharAtPoint(up) !== ' ') {
            return false;
        }

        let left = up.move(LEFT);

        if (this.grid.isNodeInside(left)) {
            if (this.isMayFalling(this.getCharAtPoint(left))) {
                return true;
            }
        }

        let right = up.move(RIGHT);
        if (this.grid.isNodeInside(right)) {
            if (this.isMayFalling(this.getCharAtPoint(right))) {
                return true;
            }
        }
    ...
    isDangerAround(direction) {
        ...
        if (this.isRoundFalling(this.player)) {
            return true;
        }

        if (this.isRoundFalling(this.player.move(direction))) {
            return true;
        }
